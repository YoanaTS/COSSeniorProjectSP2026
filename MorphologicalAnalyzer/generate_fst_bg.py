...
def safe_name(word):
    return "".join(str(ord(c)) if ord(c) > 127 else c for c in word)
...
def get_word_list():
    try:
        with open("bulgarian_words.csv", encoding="utf-8") as f:
            reader = csv.reader(f)
...
def load_stems_bg():
    ...
    stanza.download("bg")
    nlp = stanza.Pipeline("bg", processors="tokenize,pos")
    ...
    for word in words:
        ...
        for s in doc.sentences:
            for w in s.words:
                ...
                if pos == "NOUN":
                    nouns.append((lemma, "m"))
                elif pos == "VERB":
                    verbs.append((lemma, 1))
                elif pos == "ADJ":
                    adjs.append(lemma)

    return list(set(nouns)), list(set(verbs)), list(set(adjs))
...
def generate():
    ...
    with open("bulgarian_morphology.fst", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
